 ...
 *
 * Every LED frame must carry the mandatory 111xxxxx prefix, and the sacrificial dummy LED
 * has to be a valid frame (0xE0000000) - a plain all-zero word there reads as another start
 * frame and desyncs the whole chain, leaving the strip dark.
 */
void HW_NeoPixel_SPI::initBufferFraming()
{
    ...
    {
        _inst->buffer[offset] = 0xE0; // brightness 0 => dummy LED stays dark
...
void HW_NeoPixel_SPI::clear()
{
    ...
    if (_inst->hasGlobalBrightness)
    {
        // Only blank the RGB bytes - zeroing the whole frame would also destroy the
        // mandatory 111xxxxx prefix of each LED frame and desync the chain.
        uint32_t dataOffset = _inst->startFrameCount * 4 + ((_inst->dummyLedMode == 1) ? 4 : 0);
        for (uint16_t i = 0; i < _inst->ledCount; i++)
        {
            uint32_t offset = dataOffset + (uint32_t)i * 4;
